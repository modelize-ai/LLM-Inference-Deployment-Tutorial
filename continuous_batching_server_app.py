...
from server.continuous_batching_server import get_server, Server, ServerConfig
...
APP_NAME = "LLM-Inference-CB-Server"
...
def build_app(
    model_id: str = None,
    server_config_file_path: str = "cb_server_config.json",
...
async def execute_completion(request_inputs: HuggingFaceCompletionInputs):
    server = get_server()
    outputs, error, status_code, wall_time = await server.wait_task_done(request_inputs)
    if status_code != 200:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--server_config_file_path", type=str, default="cb_server_config.json")
